...
```{r}
    ...
    cat(sprintf("The interval with the greatest average number of steps is %.0f", big), "\n")    
```
...
```{r}
    ...
    cat(sprintf("Mean   (revised): %.2f    Change in Mean   = %.02f", revised_meansteps,d_mean), "\n")
    cat(sprintf("Median (revised): %.2f    Change in Median = %.02f", revised_medsteps,d_median),  "\n")
    hist(revised_dailytotal,xlab="Total Number of Daily Steps (means used for missing data)",ylab="Frequency",main="Histogram of Total Number of Daily Steps")    
```
...
```{r}
    ...
    oldvalues <- c("Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday")
    ...
    plot(x2,y2,xlab="Time Intervals",ylab="Number of Steps",type="l",main="Weekday")
```